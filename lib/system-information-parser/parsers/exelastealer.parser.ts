...
/**
 * Parse ExelaStealer log (Windows systeminfo format)
...
export function parseExelaStealer(content: string, _fileName: string): ParsedLogData {
  ...
  for (const line of lines) {
    ...
    if (lowerLine.startsWith('os name:') && !osName) {
      osName = extractValue(normalizedLine);
    }

    // OS Version: 10.0.19044 N/A Build 19044
    if (lowerLine.startsWith('os version:') && !osVersion) {
      osVersion = extractValue(normalizedLine);
    }
    ...
    // Processor(s): 1 Processor(s) Installed. [01]: Intel64 Family 6 Model 85 Stepping 7 GenuineIntel 2800 Mhz
